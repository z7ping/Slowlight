...
using GetSystemMetricsForDpiFn = int(WINAPI*)(int index, UINT dpi);
...
int SystemMetricForDpi(int index, UINT dpi) {
  ...
  if (user32_module != nullptr) {
    const auto get_metric = reinterpret_cast<GetSystemMetricsForDpiFn>(
        GetProcAddress(user32_module, "GetSystemMetricsForDpi"));
    if (get_metric != nullptr) {
...
HICON LoadAppIcon(int width, int height, bool shared = false) {
  UINT flags = LR_DEFAULTCOLOR;
  if (shared) {
    flags |= LR_SHARED;
  }
...
const wchar_t* WindowClassRegistrar::GetWindowClass() {
  if (!class_registered_) {
    ...
    window_class.hIcon = LoadAppIcon(GetSystemMetrics(SM_CXICON),
                                     GetSystemMetrics(SM_CYICON), true);
    window_class.hIconSm = LoadAppIcon(GetSystemMetrics(SM_CXSMICON),
                                       GetSystemMetrics(SM_CYSMICON), true);
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    ...
    case WM_DPICHANGED: {
      ...
      UpdateWindowIcons(LOWORD(wparam));
...
void Win32Window::UpdateWindowIcons(UINT dpi) {
  ...
  if (new_small != nullptr) {
    ...
    small_icon_ = new_small;
  }
}

void Win32Window::ReleaseWindowIcons() {
  if (large_icon_ != nullptr) {
    DestroyIcon(large_icon_);
    large_icon_ = nullptr;
  }
